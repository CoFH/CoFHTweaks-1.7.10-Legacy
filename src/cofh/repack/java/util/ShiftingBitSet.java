...
public class ShiftingBitSet implements Cloneable, java.io.Serializable {
	 ...
	 * represent bits with indices in the range <code>0</code> through <code>nbits-1</code>. All bits are initially <code>false</code>.
	 *
	 * @param nbits
	 ...
	 *                is negative.
	 */
	public ShiftingBitSet(int nbits) {
	...
	public void shiftRight(int fromIndex) {

		int toIndex = wordsInUse * 64;
		checkRange(fromIndex, toIndex);
		...
		if (startWordIndex == endWordIndex) {
			// Case 1: One word
			long bit = words[startWordIndex] >>> 63;
			words[startWordIndex] = (words[startWordIndex] & ~firstWordMask) |
					((words[startWordIndex] & firstWordMask) << 1);
			if (bit == 1)
				set(startWordIndex * 64 + 1);
		} else {
			// Case 2: Multiple words
			// Handle first word
			long bit = words[startWordIndex] >>> 63, temp;
			words[startWordIndex] = (words[startWordIndex] & ~firstWordMask) |
					((words[startWordIndex] & firstWordMask) << 1);
			...
			// Handle last word (restores invariants)
			temp = words[endWordIndex] >>> 63;
			words[endWordIndex] = (words[endWordIndex] << 1) | bit;
			if (temp == 1)
				set(endWordIndex * 64 + 1);
		}
	...
	public void shiftLeft(int fromIndex) {
		...
		if (startWordIndex == endWordIndex) {
			...
			// Handle last word (restores invariants)
			words[endWordIndex - 1] |= (words[endWordIndex] & 1) << 63;
			words[endWordIndex] >>>= 1;
		}
	 ...
	 * is <code>true</code> if the bit with the index <code>bitIndex</code> is currently set in this <code>BitSet</code>; otherwise, the result
	 * is <code>false</code>.
	 *
	 * @param bitIndex
	 *            the bit index.
	...
	/**
	 * Returns a new <tt>BitSet</tt> composed of bits from this <tt>BitSet</tt> from <tt>fromIndex</tt> (inclusive) to <tt>toIndex</tt> (exclusive).
	 *
	 * @param fromIndex
	 ...
	 * @since 1.4
	 */
	public ShiftingBitSet get(int fromIndex, int toIndex) {
		...
		if (len <= fromIndex || fromIndex == toIndex)
			return new ShiftingBitSet(0);

		// An optimization
		if (toIndex > len)
			toIndex = len;

		ShiftingBitSet result = new ShiftingBitSet(toIndex - fromIndex);
	...
	/**
	 * Returns the index of the first bit that is set to <code>true</code> that occurs on or after the specified starting index. If no such
	...
	/**
	 * Returns the index of the first bit that is set to <code>false</code> that occurs on or after the specified starting index.
	 *
	 * @param fromIndex
	 *            the index to start checking from (inclusive).
	...
	/**
	 * Returns true if the specified <code>BitSet</code> has any bits set to <code>true</code> that are also set to <code>true</code> in this
	 * <code>BitSet</code>.
	 *
	 * @param set
	 ...
	 * @since 1.4
	 */
	public boolean intersects(ShiftingBitSet set) {
	 ...
	 *            a bit set.
	 */
	public void and(ShiftingBitSet set) {
	 ...
	 *            a bit set.
	 */
	public void or(ShiftingBitSet set) {
	 ...
	 * statements holds:
	 * <ul>
	 * <li>The bit initially has the value <code>true</code>, and the corresponding bit in the argument has the value <code>false</code>.
	 * <li>The bit initially has the value <code>false</code>, and the corresponding bit in the argument has the value <code>true</code>.
	 * </ul>
	 *
	 * @param set
	 *            a bit set.
	 */
	public void xor(ShiftingBitSet set) {
	 ...
	 *
	 * @param set
	 *            the <code>BitSet</code> with which to mask this <code>BitSet</code>.
	 * @since 1.2
	 */
	public void andNot(ShiftingBitSet set) {
	 ...
	 * depends only on which bits have been set within this <code>BitSet</code>. The algorithm used to compute it may
	 * be described as follows.
	 * <p>
	 * Suppose the bits in the <code>BitSet</code> were to be stored in an array of <code>long</code> integers called, say, <code>words</code>, in such a manner
	 * that bit <code>k</code> is set in the <code>BitSet</code> (for nonnegative values of <code>k</code>) if and only if the expression
	 *
	 * <pre>
	 * ((k &gt;&gt; 6) &lt; words.length) &amp;&amp; ((words[k &gt;&gt; 6] &amp; (1L &lt;&lt; (bit &amp; 0x3F))) != 0)
	 * </pre>
	 *
	 * is true. Then the following definition of the <code>hashCode</code> method would be a correct implementation of the actual algorithm:
	 *
	 * <pre>
	 *
	 *
	...
	/**
	 * Returns the number of bits of space actually in use by this <code>BitSet</code> to represent bit values.
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof ShiftingBitSet))
			return false;
		if (this == obj)
			return true;

		ShiftingBitSet set = (ShiftingBitSet) obj;
	...
	public Object clone() {
		...
		try {
			ShiftingBitSet result = (ShiftingBitSet) super.clone();
